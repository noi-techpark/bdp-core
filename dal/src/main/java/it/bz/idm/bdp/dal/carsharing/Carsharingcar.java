...
public class Carsharingcar extends MeasurementStation {
	...
	private static Long getCarCount(EntityManager em, Carsharingstation station) {
		TypedQuery<Long> query = em.createQuery("Select count(basicdata) from CarsharingCarStationBasicData basicdata where basicdata.carsharingStation = :carsharing and basicdata.carsharingStation.active=true", Long.class);
