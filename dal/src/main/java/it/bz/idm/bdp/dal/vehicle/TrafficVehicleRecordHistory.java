...
public class TrafficVehicleRecordHistory {
	...
	public static final List<String[]> DATA_TYPES = new ArrayList<String[]>(){

		private static final long serialVersionUID = -4076147637743429750L;
	...
	public TrafficVehicleRecordHistory(CarValue value, Mobilestation station) {
    	...
    	if ((gps_1_long_deg != null && gps_1_long_deg != 0) && (gps_1_lat_deg!=null && gps_1_lat_deg != 0)){
    		this.position = geometryFactory.createPoint(new Coordinate(gps_1_long_deg, gps_1_lat_deg));
    	}
	}
		...
		if (dataTypeExists(type)){
			...
			for (Object object:query.getResultList()){
				...
				if (valueObject instanceof Double){
					...
					value = Math.round(value * 1000)/1000d;
					stringValue = value.toString();
				}
				Date date = (Date)objects[0];
				TrafficVehicleRecordDto dto = new TrafficVehicleRecordDto(date.getTime(), stringValue);
				dtos.add(dto);
			}
		}
		return dtos;
