...
public class DtoParser {
	...
	public static SlimRecordDto reduce(RecordDto dto) {
		...
		if (dto instanceof SimpleRecordDto) {
			...
			slimDto = new SlimRecordDto(bDto.getTimestamp(), bDto.getValue(), null, bDto.getCreated_on());
		}else if (dto instanceof ParkingRecordExtendedDto) {
			ParkingRecordExtendedDto pdto = (ParkingRecordExtendedDto) dto;
			slimDto = new SlimRecordDto(pdto.getTimestamp(),pdto.getValue(),null,pdto.getCreated_on());
		}
		return slimDto;
	}
}
