...
public class NominatimLocationLookupUtil implements LocationLookup {
	...
	/**
	 * Municipalities are described in the response DTO address field in one of this
	 * fields. If the first is missing the second one is used and so on
	 */
	...
	public String lookupLocation(Double longitude, Double latitude) throws NominatimException {
		...
		UriComponents uriComponents = UriComponentsBuilder.newInstance().scheme(NOMINATIM_SCHEME).host(NOMINATIM_HOST)
				.path(NOMINATIM_PATH).queryParams(parameters).build();
	 ...
	 * @return coordinate pair in the order longitude,latitude in EPSG 4326
	 *         projection
	...
	public Double[] lookupCoordinates(String address) throws NominatimException {
		...
		headers.add("accept", CONTENT_TYPE);
		UriComponents uriComponents = UriComponentsBuilder.newInstance().scheme(NOMINATIM_SCHEME).host(NOMINATIM_HOST)
				.path("/").queryParams(parameters).build();
		URI uri = uriComponents.toUri();
		RequestEntity<List<NominatimAddressLookupResponseDto>> requestEntity = new RequestEntity<>(headers,
				HttpMethod.GET, uri);

		ResponseEntity<NominatimAddressLookupResponseDto[]> exchange = restTemplate.exchange(requestEntity,
				NominatimAddressLookupResponseDto[].class);
		if (exchange.getStatusCode().equals(HttpStatus.BAD_GATEWAY))
			throw new NominatimException("Nominatim got to many request by" + REFERER);
